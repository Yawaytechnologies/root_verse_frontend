// src/redux/services/collectionCentreOperatorService.js
...
export const collectionCentreOperatorService = {
  /** POST /api/admin/operators/collection-centre */
  create: async (payload) => {
    const res = await api.post("/api/admin/operators/collection-centre", payload);
    return res.data;
  },

  /** GET /api/admin/users?role=COLLECTION_CENTRE_OPERATOR */
  getList: async ({ page = 1, page_size = 20, is_active } = {}) => {
    const params = { role: "COLLECTION_CENTRE_OPERATOR", page, page_size };
   ...
   */
  setStatus: async (operatorId, status) => {
