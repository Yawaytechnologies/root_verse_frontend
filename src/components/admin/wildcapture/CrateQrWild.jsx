...
import {
  ...
  selectCrateQrWildCreateError,
  selectCrateQrWildCreateLoading,
  selectCrateQrWildDistricts,
  selectCrateQrWildDistrictsError,
  selectCrateQrWildDistrictsLoading,
  selectCrateQrWildQrs,
  selectSelectedCrateQrWild,
  setSelectedCrateQrWild,
} from "../../../redux/reducer/crateQrWildSlice";
import {
  createCrateQrWildBatch,
  fetchCrateQrWildDistricts,
} from "../../../redux/action/crateQrWildActions";

import { Eye, Download, RefreshCw, Copy } from "lucide-react";
import QRCode from "qrcode";
import jsPDF from "jspdf";

export default function CrateWildQrGenerator() {
  const dispatch = useDispatch();

  const districts = useSelector(selectCrateQrWildDistricts);
  const districtsLoading = useSelector(selectCrateQrWildDistrictsLoading);
  const districtsError = useSelector(selectCrateQrWildDistrictsError);

  const qrs = useSelector(selectCrateQrWildQrs);
  const createLoading = useSelector(selectCrateQrWildCreateLoading);
  const createError = useSelector(selectCrateQrWildCreateError);

  const selected = useSelector(selectSelectedCrateQrWild);
  ...
  useEffect(() => {
    dispatch(fetchCrateQrWildDistricts());
  }, [dispatch]);

  // reset to page 1 when new batch arrives / cleared
  useEffect(() => {
    setPage(1);
  }, [qrs.length]);
  ...
  const districtLabel = (d) =>
    `${d.name} (${d.district_code}) • ${d.state_name} • ${d.country_code}`;
  ...
  const onGenerate = async () => {
    dispatch(clearCrateQrWildStatus());

    const c = Number(count);
    const did = Number(districtId);
    if (!did || !c) return;

    await dispatch(createCrateQrWildBatch({ type, count: c, districtId: did }));
    setCount("");
  };

  const onClearAll = () => {
    dispatch(clearCrateQrWildBatch());
    setDistrictId("");
    setCount("");
    setPage(1);
  };
  ...
  const openPreview = async (qr) => {
    ...
    try {
      const url = await makeQrDataUrl(qr.code, 700);
      setPreviewUrl(url);
      setPreviewOpen(true);
      dispatch(setSelectedCrateQrWild(qr));
    } finally {
      setPreviewLoading(false);
    }
  };

  // index here must be global index (not page index)
  const downloadSinglePng = async (qr, globalIndex) => {
    if (!qr?.code) return;
    const url = await makeQrDataUrl(qr.code, 900);
    downloadDataUrl(url, `QR_${globalIndex + 1}.png`);
  };

  // ✅ PDF: ONE QR PER PAGE, QR IMAGE ONLY (NO TEXT/NUMBERING)
  const downloadAllPdf = async () => {
    ...
    for (let i = 0; i < qrs.length; i++) {
      const img = await makeQrDataUrl(qrs[i].code, 900);
      if (i > 0) doc.addPage();
      doc.addImage(img, "PNG", x, y, qrSize, qrSize);
    }

    doc.save(`crate_wild_qrs_${Date.now()}.pdf`);
  };
  ...
  return (
    <div className="w-full px-2 md:px-4 py-4">
      <div className="rounded-3xl border border-slate-200/80 bg-white/75 backdrop-blur shadow-[0_12px_40px_-28px_rgba(2,6,23,0.35)] overflow-hidden">
        ...
        <div className="px-5 md:px-6 py-5 border-b border-slate-200/70 bg-gradient-to-r from-emerald-50/70 via-white/60 to-white">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Reserve crate QRs in bulk. Backend continues numbering.
              </p>
            </div>

            <button
              type="button"
              onClick={() => dispatch(fetchCrateQrWildDistricts())}
          ...
          {(districtsError || createError) && (
            <div className="mt-4 space-y-2">
              ...
              {createError && (
                <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm">
                  <div className="font-semibold text-red-800">Create Error</div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-slate-500 mt-1">
                  {qrs.length ? `${qrs.length} QR(s) generated` : "No QRs generated yet"}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-end w-full sm:w-auto">
                ...
                <button
                  ...
                  title="Download all QRs as PDF (1 QR per page)"
                >
                ...
                <>
                  ...
                  <div className="md:hidden space-y-3 p-3">
                    {pageItems.map((qr, idx) => {
                      ...
                      const active = selected?.code === qr.code;
                      ...
                      return (
                        <div
                          ...
                          className={[
                            ...
                            active ? "ring-2 ring-emerald-200 bg-emerald-50/40" : "",
                          ].join(" ")}
                        >
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-2">
                            ...
                            >
                              <Download className="h-4 w-4 shrink-0" />
                              <span className="truncate whitespace-nowrap">Download</span>
                            </button>
                          </div>
                        </div>
                      );
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {pageItems.map((qr, idx) => {
                          ...
                          return (
                            <tr
                              key={qr.code}
                              className={active ? "bg-emerald-50/60" : "hover:bg-slate-50"}
                            >
                              ...
                              <td className="px-4 py-3 text-sm text-slate-600">{qr.id}</td>
                    ...
                    <div className="px-3 pb-3">
                      <div className="mt-3 flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div className="text-sm text-slate-600">
                          Showing {startIdx + 1}–{Math.min(endIdx, qrs.length)} of {qrs.length}
                        </div>
        ...
        {/* ✅ QR PREVIEW MODAL (QR shown ONLY here) */}
        {previewOpen && (
