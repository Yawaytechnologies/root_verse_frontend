...
function StatusSelect({ value, disabled, onChange, className = "" }) {
  ...
  return (
    <select
      ...
      className={[
        // ✅ compact width + tighter padding (fix empty space)
        "h-10 w-[135px] rounded-2xl border pl-3 pr-8 text-xs font-extrabold outline-none",
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={onPrev}
          disabled={page <= 1}
          className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-white px-3 py-2 text-xs font-extrabold text-emerald-800 hover:bg-emerald-50 disabled:opacity-50"
        >
...
export default function VesselRegistry() {
  ...
  // optimistic dropdown values
  const [statusOverride, setStatusOverride] = useState({}); // { [id]: "PENDING" | "APPROVED" }
  ...
  // global search filter for both sections
  ...
  const pendingRows = useMemo(() => {
    return searched.filter(
      (v) => String(v.approval_status || "").toUpperCase() === "PENDING"
    );
  }, [searched]);

  const approvedRows = useMemo(() => {
    return searched.filter(
      (v) => String(v.approval_status || "").toUpperCase() === "APPROVED"
    );
  }, [searched]);
  ...
  const changeStatus = async (v, nextStatus) => {
    ...
    if (!STATUS_OPTIONS.includes(next)) return;
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-white px-4 py-2 text-xs font-extrabold text-emerald-800 hover:bg-emerald-50"
                      >
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6">
      ...
      {/* PENDING SECTION */}
      <div className="mt-5 overflow-hidden rounded-3xl border border-emerald-100 bg-white shadow-sm">
        <div className="p-4 sm:p-5 border-b border-emerald-100 bg-emerald-50/30">
          <SectionHeader
