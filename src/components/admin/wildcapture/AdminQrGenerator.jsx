...
import { FiCopy, FiRefreshCcw, FiAlertTriangle, FiEye } from "react-icons/fi";
...
import {
  ...
  setSelectedCode,
  clearError,
} from "../../../redux/reducer/qrSlice"; // ✅ adjust if needed
import { reserveBulkQrs } from "../../../redux/action/qrActions"; // ✅ adjust if needed
...
export default function AdminQrGenerator() {
  ...
  const { loading, error, type, count, lastBatch, selectedCode, lastBatchMeta } =
    useSelector((s) => s.qr);

  // ✅ Always keep selectedCode valid after new batch load
  useEffect(() => {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {/* ✅ Generate button near form */}
            <button
              type="button"
              onClick={onGenerate}
              disabled={loading}
              className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white shadow
                ${
                  loading
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {/* ✅ Key: allow scroll only on small screens, NOT on md+ */}
          <div className="w-full overflow-x-auto md:overflow-x-hidden">
            <table className="w-full table-fixed">
              <thead className="bg-slate-100">
                <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-600">
                  {/* widths adjust by breakpoint so md fits without overflow */}
                  ...
                  {/* hide Created on md to ensure it fits; show on lg+ */}
              ...
              <tbody className="divide-y divide-slate-200">
                {lastBatch.length === 0 ? (
                  ...
                  lastBatch.map((it) => (
                    ...
                    >
                      {/* ✅ Truncate long codes so md never overflows */}
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell text-sm text-slate-700">
                        {/* short-ish created to avoid width issues */}
                        {it.created_at ? new Date(it.created_at).toLocaleString() : "—"}
                      </td>

                      {/* ✅ Actions: icons on md, optional text only on xl */}
                      <td className="px-4 py-3">
                        <div
