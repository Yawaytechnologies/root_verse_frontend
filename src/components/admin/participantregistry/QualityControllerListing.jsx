// QualityControllerListing.jsx — QC Listing Page (PCC Admin)
...
const EMPTY = {
  checker_name: "", checker_email: "", checker_phone: "",
  state_id: "", district_id: "", location_id: "", is_active: true,
};
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-stone-50 px-4 py-3 ring-1 ring-stone-200">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-stone-500 mt-0.5">Toggle active / inactive</p>
      </div>
      <button
        type="button"
      ...
      >
        <span className={`pointer-events-none absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform duration-200 ${checked ? "translate-x-5" : "translate-x-0"}`} />
      </button>
    </div>
  );
}
...
function StatusBadge({ active }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 ${active ? "bg-emerald-50 text-emerald-700 ring-emerald-200" : "bg-stone-100 text-stone-500 ring-stone-200"}`}>
...
/* ── Main ── */
export default function QualityControllerListing() {
  ...
  /* ── Render ── */
  return (
    <div className="pccQCList min-h-full" style={{ background: "#F7F5F2" }}>
      <style>{`.pccQCList * { box-sizing: border-box; }`}</style>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 space-y-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-stone-900">QC Listing</h1>
            <p className="mt-1 text-sm text-stone-500">View, search and manage quality checker accounts</p>
          </div>
        ...
        <div className="rounded-2xl bg-white ring-1 ring-stone-200 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 border-b border-stone-100 px-5 py-4">
            ...
            <div>
              <p className="font-semibold text-stone-800">View by QC Code</p>
              <p className="text-xs text-stone-500">Search and open a checker by their unique code</p>
            </div>
          </div>
          <div className="px-5 py-4 flex items-center gap-3">
            <div className="flex flex-1 items-center gap-2 rounded-xl bg-stone-50 px-3 ring-1 ring-stone-200 focus-within:ring-2 transition max-w-sm"
                 style={{ "--tw-ring-color": A }}>
            ...
            <button
              ...
              disabled={searching}
              className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition active:scale-[0.97] disabled:opacity-60 shrink-0"
              style={{ background: A }}
            >
          ...
          {searchError && (
            <div className="mx-5 mb-4 flex items-center gap-2 rounded-xl bg-rose-50 px-3 py-2.5 text-sm text-rose-700 ring-1 ring-rose-200">
                ...
                <div key={r.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2"><StatusBadge active={r.is_active} /></div>
                    </div>
                    <div className="flex flex-col gap-2">
                      <button onClick={() => openRowView(r)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-stone-100 text-stone-600 ring-1 ring-stone-200 hover:bg-stone-200 transition">
                        <FiEye className="h-4 w-4" />
                      </button>
                      <button onClick={() => openEditModal(r)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-stone-100 text-stone-600 ring-1 ring-stone-200 hover:bg-stone-200 transition">
                        <FiEdit2 className="h-4 w-4" />
                      </button>
                      <button onClick={() => doDelete(r)} disabled={!!deletingById?.[r.id]} className="flex h-9 w-9 items-center justify-center rounded-xl bg-rose-50 text-rose-600 ring-1 ring-rose-200 hover:bg-rose-100 transition disabled:opacity-50">
            ...
            <table className="w-full text-sm border-separate border-spacing-0 table-fixed">
              ...
              <thead>
                <tr className="bg-stone-50/80">
                  {["Checker", "Email", "Phone", "Location", "Status", "Actions"].map(h => (
                    <th key={h} className="border-b border-stone-100 px-5 py-3 text-left text-[10px] font-bold tracking-[0.14em] text-stone-400 uppercase">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {loading ? (
                  ...
                  const code = pickCode(r);
                  return (
                    <tr key={r.id} className="hover:bg-stone-50/60 transition group">
                      ...
                      <td className="border-b border-stone-100 px-5 py-3.5 align-middle text-stone-700 truncate">{r.checker_email || "—"}</td>
                      <td className="border-b border-stone-100 px-5 py-3.5 align-middle text-stone-700">{r.checker_phone || "—"}</td>
                      <td className="border-b border-stone-100 px-5 py-3.5 align-middle text-stone-600 truncate">{r.location_name || "—"}</td>
                      <td className="border-b border-stone-100 px-5 py-3.5 align-middle">
                      ...
                      <td className="border-b border-stone-100 px-5 py-3.5 align-middle">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            title="View"
                            className="flex h-8 w-8 items-center justify-center rounded-lg bg-stone-100 text-stone-600 hover:bg-stone-200 transition ring-1 ring-stone-200"
                          >
                            <FiEye className="h-4 w-4" />
                          </button>
                          <button
                            ...
                            title="Edit"
                            className="flex h-8 w-8 items-center justify-center rounded-lg hover:opacity-90 transition text-white"
                            style={{ background: A }}
                          >
                            <FiEdit2 className="h-4 w-4" />
                          </button>
                          <button
                            ...
                            title="Delete"
                            className="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition ring-1 ring-rose-200 disabled:opacity-50"
                          >
                            <FiTrash2 className="h-4 w-4" />
                          </button>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col">
          ...
          <div className="relative z-10 flex flex-col h-full md:h-auto md:m-auto md:max-h-[90vh] md:w-full md:max-w-2xl bg-white shadow-2xl overflow-hidden md:rounded-2xl md:ring-1 md:ring-black/10">
            <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4 shrink-0">
              ...
              <button onClick={() => { setOpenView(false); dispatch(clearSearch()); }} className="flex h-9 w-9 items-center justify-center rounded-xl bg-stone-100 text-stone-600 ring-1 ring-stone-200 hover:bg-stone-200 transition">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
              ...
              {searching ? (
                ...
                <div className="rounded-xl bg-stone-50 ring-1 ring-stone-200 px-4 py-6 text-sm text-stone-500 text-center">
                  No details loaded. Use code search or click View on a row.
                </div>
              ) : (
            ...
            <div className="shrink-0 border-t border-stone-100 px-5 py-4 bg-stone-50/60">
              <button onClick={() => { setOpenView(false); dispatch(clearSearch()); }} className="w-full rounded-xl bg-stone-200 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-300 transition">
                Close
              </button>
            </div>
          </div>
          ...
          <div className="relative z-10 flex flex-col h-full md:h-auto md:m-auto md:max-h-[90vh] md:w-full md:max-w-2xl bg-white shadow-2xl overflow-hidden md:rounded-2xl md:ring-1 md:ring-black/10">
            <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-500 font-mono">ID: {activeRow?.id}</p>
                </div>
              </div>
              <button onClick={closeEdit} className="flex h-9 w-9 items-center justify-center rounded-xl bg-stone-100 text-stone-600 ring-1 ring-stone-200 hover:bg-stone-200 transition">
                <FiX className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={submitEdit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Select label="State"         value={editForm.state_id}    onChange={handleEditStateChange}                               loading={statesLoading}    error={statesError}    options={stateOptions}    required />
                <Select label="District"      value={editForm.district_id} onChange={v => setEditForm(p => ({ ...p, district_id: v }))}   loading={districtsLoading} error={districtsError} options={districtOptions} required disabled={!editForm.state_id} />
                <Select label="Location (Port)" value={editForm.location_id} onChange={v => setEditForm(p => ({ ...p, location_id: v }))} loading={locationsLoading} error={locationsError} options={locationOptions} required disabled={!editForm.state_id} />
              </div>

              <StatusToggle checked={!!editForm.is_active} onChange={v => setEditForm(p => ({ ...p, is_active: v }))} />

              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={closeEdit} className="rounded-xl bg-stone-100 px-5 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-200 transition">
                  Cancel
                </button>
